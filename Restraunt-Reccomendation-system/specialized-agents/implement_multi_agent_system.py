...
def node_analyze_styles(state: dict) -> dict:
    ...
    user_message = f"""Analyze the food styles, cuisine types, regional variations, cooking methods, and flavor profiles 
in the following recommendations, and align them with the user's preferences.
...
Tasks:
- Identify dominant cuisine types and regional variations
- Detect cooking methods (grilling, steaming, frying, etc.)
- Extract flavor profiles (spicy, umami-rich, sweet, acidic, etc.)
- Match food styles with user preferences
- Highlight best alignment between user taste and available options
...
"""
    ...
    try:
        ...
        print(f"✓ Style analysis completed with {len(style_analysis.get('styles', []))} items")
    except Exception as e:
