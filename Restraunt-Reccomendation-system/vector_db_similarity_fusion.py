...
image_db   = Chroma(collection_name="images",          persist_directory=DB_DIR)
...
# Optional filters (adjust if needed)
where_articles = None
where_images = None
...
if len(rows_text_heavy) == 0:
    print("⚠️ No results found for text-heavy fusion.")
else:
    print_fused(
        rows_text_heavy,
...
if len(rows_image_heavy) == 0:
    print("⚠️ No results found for image-heavy fusion.")
else:
    print_fused(
        rows_image_heavy,
